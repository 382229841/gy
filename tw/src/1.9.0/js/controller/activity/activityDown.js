app.register.controller('activityAutumnController', function ($rootScope, $scope, httpRequest, dataStringify, signSha1, analytics, $location, $window, $routeParams) {
    //window.ontouchstart = function(e) { e.preventDefault(); };
    $scope.isShowNavbar=false;
    $scope.isApp=isApp($location);
    $scope.isShowNavbar=$scope.isApp?false:true;

    var link="http://goeasy.yhiker.com/#/activity/twGift_3?source=3&sharedTo=";
    var imgUrl="http://goeasy.yhiker.com/image/activity/twGift/3/1.jpg";
    var title="购轻松台湾";
    var desc="台湾欢迎礼";
    ...
    $scope.buyNow=function(i,c){
       ...
       if($scope.isApp){
             ...
             var actId="";
			  ...
			  if(!appToken){
			  	...
			  	window.location.href = easybuy.appActivity 
						+ "?action=19"
						+"&mustLogin=1&goodId="+$scope.products[i].goodsId
						+"&name="+$scope.products[i].goodsName
						+"&smallImg="+$scope.products[i].goodsImg
						+"&promotePrice="+$scope.products[i].price+"&num=1&category="+c;
				return;
			  }
			  var data="platform=all&activityId="+actId
						  +"&token="+appToken
						  +"&goodsId="+$scope.products[i].goodsId;
			  //alert(data);
			  httpRequest.APIPOST('/activity/validate_v1.4', dataStringify(data), { "content-type": "application/x-www-form-urlencoded" }).then(function (result) {
			...
			openDialog("该活动商品仅支持在购轻松台湾App内预订", "", arrButton, null,
				function (r) {
					if (r) {
    ...
    if($location.search()){// && $location.search().activityId){
          ...
          httpRequest.APIPOST('/activity/detail', dataStringify(data), { "content-type": "application/x-www-form-urlencoded" }).then(function (result) {
              if (result && result.code == statusCode.Success) {
                  ...
                  desc=act.desc;
                  $scope.products=[];
                  if($scope.activityInfo && $scope.activityInfo.limitBuyGoodsList){
                      $scope.products=$scope.activityInfo.limitBuyGoodsList;
                  }
...
app.register.controller('activitytwGift_V4Controller', function ($rootScope, $scope, httpRequest, dataStringify, signSha1, analytics, $location, $window, $routeParams) {
    //window.ontouchstart = function(e) { e.preventDefault(); };
    $scope.isShowNavbar=false;
    $scope.isApp=isApp($location);
    $scope.isShowNavbar=$scope.isApp?false:true;
...
app.register.controller('activitytwGift_V3Controller', function ($rootScope, $scope, httpRequest, dataStringify, signSha1, analytics, $location, $window, $routeParams) {
    //window.ontouchstart = function(e) { e.preventDefault(); };
    $scope.isShowNavbar=false;
    $scope.isApp=isApp($location);
    $scope.isShowNavbar=$scope.isApp?false:true;
...
app.register.controller('activitytwGiftController', function ($rootScope, $scope, httpRequest, dataStringify, signSha1, analytics, $location, $window, $routeParams) {
    //window.ontouchstart = function(e) { e.preventDefault(); };
    $scope.isShowNavbar=false;
    $scope.isApp=isApp($location);
    $scope.isShowNavbar=$scope.isApp?false:true;
