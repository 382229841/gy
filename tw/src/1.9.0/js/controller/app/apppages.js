app.register.controller('tourDetailController', function ($rootScope, $scope, httpRequest, dataStringify, analytics, $location, $window, $routeParams) {
    $scope.noInApp=function(){
        ...
        openDialog("仅支持在购轻松台湾App内预订", "", arrButton, null,
            function (r) {
                if (r) {
    ...
    $scope.Recommend=function(products){
        ...
        for(var i=0;i<products.length;i++){
            if($scope.isApp){            
                lis=lis+"<li style=\"\">"+
                "<a href=\""+
                easybuy.appActivity + "?action=1&goodId="+products[i].id+"&category="+products[i].category
                +"\" style=\"background-image:url("+products[i].img+")\"></a>"
                +"<span style=\"display: -webkit-box;  height: 3em; overflow: hidden;text-overflow: ellipsis;width: 100%; -webkit-line-clamp: 2; -webkit-box-orient: vertical;\">"+products[i].name+"</span>"

                +"<span class=\"promotePriceNew\" style=\"margin-top:5px;border-top:1px dashed #ddd;display:block\">￥"+products[i].price+"</span>"
                +"<div></div></li>";
            }else{
                lis=lis+"<li style=\"\">"+
                "<a href=\"#/product/"+products[i].id+"/1\" style=\"background-image:url("+products[i].img+")\"></a>"
                +"<span style=\"display: -webkit-box;  height: 3em; overflow: hidden;text-overflow: ellipsis;width: 100%; -webkit-line-clamp: 2; -webkit-box-orient: vertical;\">"+products[i].name+"</span>"
        ...
        $("#scroller3").css("width",products.length*40+"%");
        $("#scroller3ul li").css("width",$rootScope.winWidth*0.4+"px");
        ...
        $("#scroller3ul li div").css("height",$rootScope.winWidth*0.4-20+"px");
        $("#scroller3ul li div").css("width","2px");
        $("#scroller3ul li div").css("border-right","1px solid #ddd");
        
        //$("#scroller2ul li").css("border-right","1px solid #ddd");

        $("#scroller3ul li a").css("width",100+"%");
        $("#scroller3ul li a").css("height",$rootScope.winWidth*0.4-20+"px");
        $("#scroller3ul li a").css("display","block");
        $("#wrapper3").css("height",$rootScope.winWidth*0.4+80+"px");
        $("#scroller3").css("height",$rootScope.winWidth*0.4-20+"px");
       ...
       // $("#wrapper2").css("scrollbar-face-color","#f00");
    ...
    $scope.getRecommendGoods=function(){
         httpRequest.APIPOST('/related/get', dataStringify("platform=all&category=2&relatedId=" + $routeParams.id), { "content-type": "application/x-www-form-urlencoded" }).then(function (result) {
                if (result && result.code == statusCode.Success) {
...
app.register.controller('wifiServiceController', function ($rootScope, $scope, httpRequest, dataStringify, analytics, $location, $window, $routeParams) {
    ...
    $scope.getRecommendGoods=function(){
         httpRequest.APIPOST('/related/get', dataStringify("platform=all&category=4"), { "content-type": "application/x-www-form-urlencoded" }).then(function (result) {
                if (result && result.code == statusCode.Success) {
                    var products=result.result.list;
                    if(products && products.length>0){                        
                        $scope.Recommend(products);
